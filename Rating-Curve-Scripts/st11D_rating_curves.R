...
#merging the Depths of each stream 
		...
		dx <- sapply(index(Q11Dz), f)
		QP <- cbind(Q11D_full, light = coredata(light) [dx])
		Q11D_full <- data.frame(QP)    
...
names(depths) <- c("time","L_depthm", "L_tempC", "H_depthm", "H_tempC", "st11D_depthm", "st11D_tempC")
...
#model selection to determine the best model
	...
	Q11D_gm_mod <- lm(log(Q.mod) ~ log(d11D) + log(dL) * temp_L + log(d11D)*temp_11D, Q11D_full_mod, na.action = "na.fail")
	Q11D_MS_mod <- dredge(Q11D_gm_mod, extra = c("R^2", F = function(x) summary(x)$fstatistic[[1]]))

	
	Q11D_gm <- lm(log(Q.mod) ~ log(d11D) + temp_11D + log(dL) + temp_L + temp_H + as.factor(summer11D), Q11D_full_mod, na.action = "na.fail")
	Q11D_gm <- lm(log(Q.mod) ~ log(d11D) + temp_11D + log(dL), Q11D_full_mod, na.action = "na.fail")
	

	Q11D_MS <- dredge(Q11D_gm, extra = c("R^2", F = function(x) summary(x)$fstatistic[[1]]))
	
	subset(Q11D_MS, delta < 5)
	subset(Q11D_MS)
	subset(Q11D_MS_mod)
...
#looking at colinearity
	...
	ggplot(Q11D_full_mod, aes(x = log(dL), y = log(Q.mod))) + geom_point()

	ggplot(Q11D_full_mod, aes(x = log(dL), y = log(d11D))) + geom_point()
	
	ggplot(Q11D_full_mod, aes(x = log(d11D), y = log(Q.mod))) + geom_point()

	ggplot(Q11D_full_mod, aes(x = temp_11D, y = log(Q.mod))) + geom_point()
...
#Applying MR to predict new data in depths
	#first need to code for season
		depths$summer11D <- as.factor(ifelse(as.numeric(format(depths$time, "%m")) >= 6 & as.numeric(format(depths$time, "%m"))<= 9, 1, 0))
		depths$year11D <- as.numeric(format(depths$time, "%y"))
	#merge new files	
	depths_m <- depths
	names(depths_m) <- c("time", "dL", "temp_L", "dH", "temp_H", "d11D", "temp_11D", "year11D") #needs to be renamed so it matches the names in the equation.
