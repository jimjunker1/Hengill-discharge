...
pres5 <- read.csv("C:/Users/Jim/Documents/Projects/Iceland/Temp-Disch-Light/Working Q/Do Not Touch/9736061_ST5b.csv")
...
#Make hourly means
	...
	names(pres5hr_d) <- c("time", "depthm", "tempC")
...
##First merge all the depth data by time
	...
	pres5hr <- aggregate(pres5hr_d[c("depthm", "tempC")],
...
##  merging the Q and PT depth data
		...
		#merging Temp data
		 ...
		 ix <- vapply(index(Q5z), f, integer(1))
		 QP <- cbind(Q5_full, temp_L = coredata(temp_L)[ix])
		 Q5_full <- data.frame(QP)

 		 f <-  function(u) which.min(abs(as.numeric(index(temp_H)) - as.numeric(u)))
		 ix <- vapply(index(Q5z), f, integer(1))
		 QP <- cbind(Q5_full, temp_H = coredata(temp_H)[ix])
		 Q5_full <- data.frame(QP)
...
#Merging the Depths of each stream
		...
		dx <- sapply(index(Q5z), f)
		QP <- cbind(Q5_full, dH = coredata(dH) [dx])
		Q5_full <- data.frame(QP)

		f <- function(u) which.min(abs(as.numeric(index(light)) - as.numeric(u)))
		dx <- sapply(index(Q5z), f)
		QP <- cbind(Q5_full, light = coredata(light) [dx])
		Q5_full <- data.frame(QP)    

		f <- function(u) which.min(abs(as.numeric(index(d5)) - as.numeric(u)))
		dx <- sapply(index(Q5z), f)
		QP <- cbind(Q5_full, cum.light = coredata(lightmod) [dx])
		Q5_full <- data.frame(QP)  

#adding in a season variable

summer5 <- 	ifelse(as.numeric(format(Q5_full$Pd, "%m")) >= 6 & as.numeric(format(Q5_full$Pd, "%m"))<= 9, 1, 0) 		

Q5_full <- cbind(Q5_full, summer5)

year5 <- as.numeric(format(Q5_full$Pd, "%y"))
Q5_full <- cbind(Q5_full, year5)


					# #just check to make sure files are matched up right
				 ggplot(Qw5, aes(x = Q.mod, y = Q_DS)) + geom_point()
...
depths <- merge(depths, pres5hr[,2:4], by = "time", all = T)
depths <- depths[!as.numeric(format(depths$time, "%y")) == 1,]
names(depths) <- c("time","L_depthm", "L_tempC", "H_depthm", "H_tempC", "st5_depthm", "st5_tempC")
...
#NEED TO FIGURE OUT WHICH PD TEMP DATA TO USE
	...
	ggplot(Q5_full, aes(y = Q5_full$Q.mod, x = Q5_full$d5, label = Qdate))+
		geom_point()+
		geom_text() +
		stat_smooth(method = "lm")+
		geom_abline(intercept = 0, line = 1)
...
Q5_full$fitted <- fitted(sm_rating5)
	ggplot(Q5_full, aes(x =log(fitted), y = Q.mod))+
...
Q5_full_mod$fitted <- fitted(sm_rating5_mod)
	...
	ggplot(Q5_full_mod, aes(x =exp(fitted), y = Q.mod)) +
		geom_point(shape = 21, fill = "green", size = 3) +
		geom_abline(intercept = 0, slope = 1) +
		#xlim(1.05,1.25) +
		#ylim(15,30) +
...
save.image(file = "C:/Users/Jim/Documents/Projects/Iceland/Temp-Disch-Light/Working Q/st5/st5.RData")
load(	"C:/Users/Jim/Documents/Projects/Iceland/Temp-Disch-Light/Working Q/st5/st5.RData")
...
F1 = 	ggplot(depths, aes(x = time, y = st5_depthm)) +
		geom_point(size = 3) +
		ggtitle("All depth measurements")


F2 = ggplot(Q5_full_mod, aes(y = Q.mod, x = d5, label= Qdate))+
		geom_point()+
		geom_text() +
...
F3 = ggplot(Q5_full_mod, aes(x = exp(fitted), y = Q.mod))+
		geom_point(shape = 21, fill = "green", size = 3)+
		geom_abline(intercept = 0, slope = 1)+
		xlim(15,30)+
		ylim(15,30)+
...
pdf("Stream5plots.pdf", height = 15, width = 8)
