...
##First merge all the depth data by time
	...
	pres9hr <- aggregate(pres9hr_d[c("depthm", "tempC")],
...
#merging Temp data
		 ...
		 ix <- vapply(index(Q9z), f, integer(1))
		 QP <- cbind(Q9_full, temp_L = coredata(temp_L)[ix])
		 Q9_full <- data.frame(QP)

 		 f <-  function(u) which.min(abs(as.numeric(index(temp_H)) - as.numeric(u)))
		 ix <- vapply(index(Q9z), f, integer(1))
		 QP <- cbind(Q9_full, temp_H = coredata(temp_H)[ix])
		 Q9_full <- data.frame(QP)
...
#merging the Depths of each stream 
		 ...
		 dx <- sapply(index(Q9z), f)
		 QP <- cbind(Q9_full, dL = coredata(dL) [dx])
		 Q9_full <- data.frame(QP)
...
depths <- merge(depths, pres9hr[,2:4], by = "time", all = T)
...
ggplot(depths, aes(x = st9_tempC, y = st9_depthm))+
		geom_point()+
		stat_smooth(method = "lm")+
		geom_abline(intercept = 0, slope = 1)+
		ylim(0,0.5)

ggplot(depths, aes(x = L_tempC, y = st9_depthm))+
		geom_point()+
		stat_smooth(method = "lm")+
...
#NEED TO FIGURE OUT WHICH PD TEMP DATA TO USE
	...
	ggplot(Q9_full, aes(x = temp_US_m, y = temp_DS_m))+
		geom_point()+
		stat_smooth(method = "lm")+
		geom_abline(intercept = 0, slope = 1)+
		xlim(0,20)+
	...
	ggplot(Q9_full, aes(x = temp_US_m, y = temp_9))+
		geom_point()+
		stat_smooth(method = "lm")+
		geom_abline(intercept = 0, slope = 1)+
		xlim(0,20)+
	...
	ggplot(Q9_full, aes(y = temp_DS_m, x = temp_9))+
		geom_point()+
		stat_smooth(method = "lm")+
		geom_abline(intercept = 0, slope = 1)+
		xlim(0,23)+
	...
	ggplot(Q9_full, aes(y = Q.mod, x = d9))+
		geom_point()+
		stat_smooth(method = "lm")+
		geom_abline(intercept = 0, slope = 1)
...
# # 				
	...
	Q9_full_mod$fitted <- fitted(sm_rating9)
	ggplot(Q9_full_mod, aes(x = fitted, y =log(Q.mod)))+
		geom_point()+
		geom_abline(intercept = 0, slope = 1)
	
	ggplot(Q9_full_mod, aes(x =exp(fitted), y = Q.mod))+
...
##ST9
	...
	#export data
	write.csv(depths_m, file = "C:/Users/Jim/Documents/Projects/Iceland/Temp-Disch-Light/Working Q/st9/depths_m9.csv")
	

save.image(file = "C:/Users/Jim/Documents/Projects/Iceland/Temp-Disch-Light/Working Q/st9/st9.RData")
load(	"C:/Users/Jim/Documents/Projects/Iceland/Temp-Disch-Light/Working Q/st9/st9.RData")


F1 = 	ggplot(depths, aes(x = time, y = st9_depthm)) +
		geom_point(size = 3) +
		ggtitle("All depth measurements")


F2 = ggplot(Q9_full_mod, aes(y = Q.mod, x = d9, label= Qdate))+
...
F3 = ggplot(Q9_full_mod, aes(x =exp(fitted), y = Q.mod))+
		geom_point(shape = 21, fill = "green", size = 3)+
		geom_abline(intercept = 0, slope = 1)+
		xlim(0,8)+
		ylim(0,8)+
...
F4 = 		ggplot(depths_m[depths_m$Q_mr <=8,], aes(x = time, y = Q_mr)) +
		geom_line(color = "blue", size = 0.25)+
		geom_point(data =  Q9_full, aes(x = Pd, y = Q.mod), shape = 21, fill = "red")+
		xlab("Date")+
		ylab(expression(paste("Q (L ", s^1,")")))+
		scale_x_datetime(breaks = "6 months", labels = date_format("%b-%y")) +
		ggtitle("Model good when Q < ~8 l/s")

pdf("Stream9plots1.pdf", height = 15, width = 8)
grid.arrange(F1, F2, F3, F4, nrow = 4, ncol = 1)
dev.off()	


F5 = 	ggplot(depths, aes(x = time, y = st9_depthm)) +
		geom_point(size = 3) +
		ggtitle("All depth measurements")


F6 = ggplot(Q9_full, aes(y = Q.mod, x = d9, label= Qdate))+
		geom_point( size = 3, fill = "green", pch = 21, )+
		geom_text() +
		scale_color_brewer(palette = "Set1") +
		#stat_smooth(method = "lm")+
...
data.labels2 <- data.frame(
	fitted = 5,
	Q.mod = 22,
	label = "R^2 = 0.78 \n p < 0.001 \n eqn = log(Q) ~ log(depth)")

F7 = ggplot(Q9_full, aes(x =exp(fitted), y = Q.mod))+
		geom_point(shape = 21, fill = "green", size = 3)+
		geom_abline(intercept = 0, slope = 1)+
		xlim(0,25)+
		ylim(0,25)+
		...
		ylab(expression(paste("Measured DS Q (L",s^-1,")")))+
		xlab(expression(paste("Fit DS Q (L",s^-1,")"))) +
		geom_text(data = data.labels2, aes(x = fitted, y = Q.mod, label = label)) +
		ggtitle("Q - Fitted")

F8 = ggplot(depths_m, aes(x = time, y = Q_mr)) +
		geom_line(color = "blue", size = 0.25)+
		geom_point(data =  Q9_full, aes(x = Pd, y = Q.mod), shape = 21, fill = "red")+
		xlab("Date")+
		ylab(expression(paste("Q (L ", s^-1,")")))+
...
pdf("Stream9plots2.pdf", height = 15, width = 8)
grid.arrange(F5, F6, F7, F8, nrow = 4, ncol = 1)
dev.off()

depths_m$crummy = as.factor(ifelse(depths_m$Q_mr >8, "C","G"))

ggplot(depths_m, aes(x = time, y = Q_mr, colour = Q_mr < 8)) +
	geom_line(size = 0.25) +
...
ggplot(depths_m, aes(x = time, y = Q_mr, group = crummy)) +
		geom_line(size = 0.25)+
		geom_point(data =  Q9_full_mod, aes(x = Pd, y = Q.mod), shape = 21, fill = "red")+
		xlab("Date")+
		ylab(expression(paste("Q (L ", s^1,")")))+
...
F5 = 		ggplot(depths_m[depths_m$Q_mr <=8,], aes(x = time, y = Q_mr)) +
		geom_line(color = "blue", size = 0.25)+
		geom_point(data =  Q9_full, aes(x = Pd, y = Q.mod), shape = 21, fill = "red")+
		xlab("Date")+
		ylab(expression(paste("Q (L ", s^1,")")))+
...
F6 = 	ggplot(depths, aes(y = Q.mod, x = d9, color = as.factor(year9))) +
		geom_point(size = 5) +
		geom_text() +
		stat_smooth(method = "lm") +
		ggtitle("All depth measurements")
...
#Feb-6-2015. added full date-time file and merged to get 15min intervals from Jul 2010
	...
	ggplot(Q9_full, aes(x = d9, y = Qtop_to_bottom, colour = Q_DS, label = Qdate)) +
