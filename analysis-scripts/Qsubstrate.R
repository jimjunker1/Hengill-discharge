...
#st1_RBS = (0.155*sqrt(median(sediment$Size[which(sediment$Stream == "st1")])))/(0.7*mean(pres_allhr$st1_vel,na.rm = T))
#st1_RBS
#mean(pres_allhr$st1_vel,na.rm = T)
...
#st5_RBS = (0.155*sqrt(median(sediment$Size[which(sediment$Stream == "st5")])))/(0.7*median(pres_allhr$st5_vel, na.rm = T))
...
st6_RBS = (0.155*sqrt(median(sediment$Size[which(sediment$Stream == "st6")])))/(0.7*mean(pres_allhr$st6_vel,na.rm = T))
...
vel_allhr = data.frame(Pd = pres_allhr$Pd, pres_allhr[str_detect(names(pres_allhr), "_vel")])
ins_allhr = data.frame(Pd = pres_allhr$Pd, pres_allhr[str_detect(names(pres_allhr), "_ins")])
